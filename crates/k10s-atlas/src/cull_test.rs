//! What the cull promises, checked against scenes built by hand.
//!
//! The cull is the hottest code in the repo and every consumer monomorphises
//! it, so the suite lives beside it rather than inside it: a `#[cfg(test)]`
//! module compiles into no binary a benchmark or the app ever links, which
//! makes moving 1,362 lines of tests out of `cull.rs` free in a way that moving
//! the implementation demonstrably is not (see benchmarks/README.md).

use crate::camera::Camera;
use crate::cull::*;
use crate::lod::{LodPolicy, StageBlend};
use crate::scene::{BlockNode, CellNode, Edge, Endpoint, Level, Rect, RegionNode, Scene, Totals};
use crate::testing::SceneSpec;

/// Where an edge endpoint is. Test-only: the cull reaches endpoints through
/// `walk_edges`, and this exists so a test can say where one *should* have been.
...
fn region_edges_are_a_spatial_index() {
    ...
    for spec in specs {
        for (built, scene) in [
            ...
            let mut grouped = 0;
            for (i, (region, range)) in scene.regions.iter().zip(&scene.region_edges).enumerate() {
                for e in &scene.edges[range.start as usize..range.end as usize] {
                    if endpoint_rect(&scene, e.a).is_none() || endpoint_rect(&scene, e.b).is_none()
                    {
...
fn scattered_edge_scene() -> Scene {
    ...
    let region_edges: Vec<std::ops::Range<u32>> = std::iter::once(0..ROW_EDGES as u32).collect();
    let mut scene = Scene {
        card_header: 26.0,
        rev: 1,
...
fn assert_index_offers_every_intersecting_child(scene: &Scene, label: &str) {
    ...
    for probe in boundary_probes(scene) {
        ...
        for region in 0..scene.regions.len() {
            ...
            blocks.clear();
            scene.for_each_region_block_candidate(region, &probe, |index, _| blocks.mark(index));
            for &index in &wanted {
                assert!(
                    blocks.seen[index],
